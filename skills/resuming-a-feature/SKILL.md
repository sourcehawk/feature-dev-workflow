...
| Which rows form a stack, and its recorded layer order (the `## Stacks` section) | The stack itself, from its one dedicated worktree (never the main checkout): `gh stack view --json` (order, PR numbers, `needsRebase`); no layer branch checked out in any other worktree, the main checkout included (`git worktree list`), except a parallel layer that has not joined yet, which stays in the temporary worktree its row records; `gh stack --version` works, or stop and ask the user to install it. If the stack exists only as a link on GitHub, with no dedicated worktree, do not adopt it here: adoption rebases and force-pushes the layers. Verify it only from its PRs for now (the PR check above), and propose the adoption as the first mutation in the Step 6 report (`feature-dev-workflow:developing-a-feature` §Linear stacks of dependent PRs go through `gh stack`). Verify it from its worktree once it is adopted |
