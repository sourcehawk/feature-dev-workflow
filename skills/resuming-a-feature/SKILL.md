...
| Which rows form a stack, and its recorded layer order (the `## Stacks` section) | The stack itself, from its one dedicated worktree (never the main checkout): `gh stack view --json` (order, PR numbers, `needsRebase`); no layer branch checked out in any other worktree, the main checkout included (`git worktree list`), except a parallel layer that has not joined yet, which stays in the temporary worktree its row records; `gh stack --version` works, or stop and ask the user to install it. A stack built with `gh stack link` has no local tracking and no stack worktree: verify its order from each layer's `baseRefName` (the PR check above) and `gh pr view <num> --json headRefName`, never by creating a worktree or running `gh stack init` for it |
