...
that happens at step 8, after the user approves the spec. `feature-dev-workflow:developing-a-feature` reuses this branch (and the
...
**REQUIRED SUB-SKILL:** Re-invoke `feature-dev-workflow:writing-github-issues` Step 2B (update) for any issue whose acceptance criteria,
...
Update it as the work progresses — see `feature-dev-workflow:developing-a-feature` for the update choreography.

Commit the spec, the plan, and the state file together as the planning artifact set on `feature/<slug>` (created in
step 2). Confirm you're not on `main`, then publish the branch so `feature-dev-workflow:developing-a-feature` can attach its integration
worktree to it:
...
`feature-dev-workflow:developing-a-feature` to start the work. The state file is the entry-point artifact for every session that touches this feature afterward.
