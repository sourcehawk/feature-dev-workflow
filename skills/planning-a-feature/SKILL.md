...
**OPTIONAL SUB-SKILL:** `simple-english:simple-english` for the prose of the spec and any ADR. If it is available, load it before you draft and write the sentences under it. It sets sentence style only: the content rules above and the ADR format still decide the sections and what they hold. The plan and the state file are written for agents and keep their own shape. If the skill is not available, do not stop or wait for it. If you have not already done so in this session, tell the user once that it can be installed with `/plugin marketplace add AminBlg/SimpleEnglish` and then `/plugin install simple-english@simple-english`. Then write short, plain, active sentences without it.
