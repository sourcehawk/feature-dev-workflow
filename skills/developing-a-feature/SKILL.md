...
- **On** — at each sub-PR's ripening, the orchestrator runs `feature-dev-workflow:copilot-review-loop` against the open sub-PR before the existing `review`-skill pass and self-merge. A comment the loop wants to push back on does not pause the fan-out; it is logged as a bubble-up concern and surfaced at the wave checkpoint.
...
- **Single-PR** — the build mode depends on the plan's task shape. Read the committed plan's task list and count the tasks that are **independent** (no task depends on another's output — they could, in principle, be built in any order). The integration task that ties them together does not count toward independence; it consumes the others.
  ...
  **OPTIONAL SUB-SKILL:** `simple-english:simple-english` for the prose of every commit body and every code or doc comment you write, here and in the Step 7 teardown commit. If it is available, load it before you draft and write the sentences under it. Where its formatting rules (headings, bold, lists, or its register for chat replies) differ from the host skill or its template, the host skill and its template win. It sets sentence style only: the subject line keeps the project's commit convention (format, prefix, length), the body still carries the why and what the diff cannot show, and the project's comment rules still decide whether a comment exists and what it may say (no narration of the code, no change history). Identifiers, strings, and log messages are code and keep the project's rules. If the skill is not available, do not stop or wait for it. If you have not already done so in this session, tell the user once that it can be installed with `/plugin marketplace add AminBlg/SimpleEnglish` and then `/plugin install simple-english@simple-english`. Then write short, plain, active sentences without it.

**Keep the tracking issue true as you build.** When an implementation decision diverges from what the tracking issue (or sub-issue / epic) currently states — a changed acceptance criterion, a stated approach that no longer matches, a scope shift, a contract that shipped in a different shape — the issue must be reconciled and the decision recorded, not left to go stale until the merge. **REQUIRED SUB-SKILL:** `feature-dev-workflow:writing-github-issues` Step 2D — it records the decision as a comment (what the issue said before, what it is now, why, with a link to the commit) and reconciles the body, both under one confirmation. Do it at the decision, while the before-state and the commit are fresh, not deferred to PR-open where the delta and its reason are lost. The PR description still carries the line-level design; the issue carries the decision trail. Applies to all three build modes: in fan-out the orchestrator owns it for the epic/sub-issue divergences a sub-PR surfaces.
...
### Linear stacks of dependent PRs go through `gh stack`
...
- A stack that exists only as a link on GitHub (built with `gh stack link`, its branches managed elsewhere, or no local tracking) is adopted first: follow **Adopting a stack that was built by hand** below, in a dedicated worktree, before anything is propagated or merged through it. After that it is a local stack like any other. `gh stack link` itself is used only to join PRs opened with `gh pr create` (see **Opening the stack's PRs**) and to correct a stale base.
...
- **Fix where the code lives, not where the comment sits.** A reviewer flags code on whichever PR they happen to be reading; the branch whose PR *introduced* the code is where the fix commits. Establish ownership before editing (`git log -S '<snippet>' --format='%h %D %s' -- <file>`, then find the earliest stack branch containing that commit): a file being visible in a descendant's worktree is history inheritance, not ownership. Fixing at the point of visibility pollutes the descendant's diff with a change to code it didn't introduce, and lets the ancestor PR merge with the defect intact. **If the owning PR has already merged,** the code now lives on the trunk it merged into (`main`, or `<type>/<slug>` in the feature-branch model), so the fix is its own small PR against that trunk, built in its own worktree off the trunk (never the stack's worktree or the main checkout). It references a new bug issue that links the owning sub-issue, since that sub-issue usually closed with its merge. It is not a commit added to whichever open layer the reviewer happened to be reading. One exception: when an open layer cannot pass its gates or behave correctly without the fix, commit the fix in the lowest open layer that needs it, and say so in that PR's body. In a `gh stack` stack, the open layers pick up the merged fix with `gh stack sync` once it lands.
- **Propagate through `gh stack` in a stack; merge forward everywhere else.** In a linear stack, check out the owning layer in the stack's worktree (`gh stack checkout <branch>`), commit the fix, run `gh stack rebase --upstack`, then `gh stack push`, and record the new head SHAs from the remote (see §Linear stacks of dependent PRs go through `gh stack`). A stack that exists only as a link on GitHub is adopted into its dedicated worktree first (see §Linear stacks of dependent PRs go through `gh stack`). Outside a stack, never rebase: once the origin branch is fixed and pushed, each descendant merges its updated parent in order and pushes, because rebasing rewrites SHAs under live review and orphans the reviewer's comment anchors.
...
**OPTIONAL SUB-SKILL:** `simple-english:simple-english` for the prose of the distilled decision records. If it is available, load it before you draft and write the sentences under it. Where its formatting rules (headings, bold, lists, or its register for chat replies) differ from the host skill or its template, the host skill and its template win. It sets sentence style only: the house format still decides the sections and what they hold. If the skill is not available, do not stop or wait for it. If you have not already done so in this session, tell the user once that it can be installed with `/plugin marketplace add AminBlg/SimpleEnglish` and then `/plugin install simple-english@simple-english`. Then write short, plain, active sentences without it.
