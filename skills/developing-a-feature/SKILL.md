...
- For branches that are managed elsewhere and must stay there, `gh stack link <bottom> ... <top>` builds the stack through the API with no local tracking and no stack worktree; its `## Stacks` entry says `linked` in place of the worktree.
