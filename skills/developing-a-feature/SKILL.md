...
**One checkout per stack.** `gh stack` checks the stack's branches out in turn, and git refuses a branch that is checked out in another worktree. So the whole stack lives in a single checkout, recorded as the worktree path of every layer's row in the state file. A separate worktree for one layer is allowed only while a brand-new layer is built in parallel; remove that worktree before the branch joins the stack. To join it, run `gh stack unstack --local` and then `gh stack init --base <trunk> <bottom> ... <new-top>` in the stack checkout (`init` adopts branches that already exist), then `gh stack submit --auto` to open its PR and link it. For branches that are managed elsewhere and must stay there, `gh stack link <bottom> ... <top>` builds the stack through the API with no local tracking.
...
3. Remove the per-layer worktrees, and create or keep the one stack checkout.
...
- **Propagate through `gh stack` in a stack; merge forward everywhere else.** In a linear stack, check out the owning layer (`gh stack checkout <branch>`), commit the fix, run `gh stack rebase --upstack`, then `gh stack push`, and record the new head SHAs from the remote (see §Linear stacks of dependent PRs go through `gh stack`). Outside a stack, never rebase: once the origin branch is fixed and pushed, each descendant merges its updated parent in order and pushes, because rebasing rewrites SHAs under live review and orphans the reviewer's comment anchors.
...
| "The function is right here in this worktree, I'll fix it here"      | Visibility is history inheritance, not ownership. Trace which PR introduced the code and fix there, then propagate (`gh stack rebase --upstack` in a stack, merge forward otherwise) — or the ancestor merges with the defect intact. |
...
| "I'll keep one worktree per layer, it's tidier" | `gh stack` checks every layer out in turn, and git refuses a branch held by another worktree. One checkout per stack; a per-layer worktree only while a brand-new layer is built in parallel. |
