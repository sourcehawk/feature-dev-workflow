...
5. **Address, then always reply and resolve.** Work one comment at a time, and before moving to the next, close the loop on it in this order: (a) apply the fix if the comment is correct, testing it, and commit it; (b) post a threaded reply stating the resolution — what you changed (name the fix or commit) for ones you applied, or why the comment does not apply for ones you reject; (c) resolve the thread (`resolveReviewThread`). The reply is mandatory and is never gated behind a confirmation prompt (see GitHub-mutation discipline) — every comment gets one, including rejected ones. **Never resolve a thread without a reply, and never silently apply or silently ignore** — a resolved-but-unexplained thread destroys the reasoning trail. For a comment that is wrong or a judgment call, the reply still goes up; handle any further pushback per the calling context (below).
