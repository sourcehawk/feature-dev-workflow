...
As each subagent surfaces its worktree path and branch, the orchestrator fills in the row in the state file's `## PRs / worktrees` table. When a subagent opens its draft PR, or `gh stack submit --auto` opens a stack's PRs, the orchestrator fills in the PR column with the base ref (`#<num> → <type>/<slug>` or `#<num> → main`, per the `sub_pr_target` setting; a stack layer above the bottom shows its parent's branch) and flips status to `draft`.
