...
Each dispatch prompt also carries this instruction for the subagent: **OPTIONAL SUB-SKILL:** `simple-english:simple-english` for the prose of your commit bodies, code and doc comments, and PR body. If it is available, load it before you draft and write the sentences under it. It sets sentence style only: the commit subject keeps the project's commit convention, and the project's comment rules still decide whether a comment exists and what it may say. If it is not available, do not stop or wait for it: write short, plain, active sentences and say in your report that it was missing. The orchestrator, not the subagent, gives the user the install hint (`/plugin marketplace add AminBlg/SimpleEnglish`, then `/plugin install simple-english@simple-english`), at most once per session.
