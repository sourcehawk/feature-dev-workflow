...
Each dispatch prompt also names `simple-english:simple-english` as an optional sub-skill for the subagent's commit bodies, code and doc comments, and PR body prose, with the scope `feature-dev-workflow:developing-a-feature` gives it: sentence style only, the commit subject keeps the project convention, and the project's comment rules still decide whether a comment exists. A subagent without the skill writes plain sentences and does not block on it. The orchestrator gives the user the install hint, at most once per session.
