...
**A sub-PR based on another sub-PR's branch makes a stack, not a wave member.** That covers a `stub-on-producer-branch` consumer and, in either `sub_pr_target` model, any strictly sequential chain whose PRs open before their parents merge. Its branches, bases, propagation, and merges go through `gh stack`, which needs one checkout per stack: follow `feature-dev-workflow:developing-a-feature` §Linear stacks of dependent PRs go through `gh stack`, including its availability check, before creating the consumer's branch. A layer's subagent works in the stack checkout (its isolation check names that path and the layer's branch), and the orchestrator runs the `gh stack` commands that create layers and open or link their PRs. Parallel sub-PRs based directly on `<base-ref>` stay in the worktree-per-sub-PR model below.
...
As each subagent surfaces its worktree path and branch, the orchestrator fills in the row in the state file's `## PRs / worktrees` table. When a subagent opens its draft PR, the orchestrator fills in the PR column with the base ref (`#<num> → <type>/<slug>` or `#<num> → main`, per the `sub_pr_target` setting) and flips status to `draft`.
