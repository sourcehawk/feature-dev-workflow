...
- The `<type>/<slug>` integration branch and the main feature worktree exist (set up by `feature-dev-workflow:developing-a-feature` before invoking this skill).
...
First classify each sub-PR in the wave. A sub-PR is a stack layer when it is based on another sub-PR's branch, or when another sub-PR is or will be based on its branch (the stack's bottom layer; a producer with a `stub-on-producer-branch` consumer in a later wave counts). A stack layer gets no worktree from the loop below, and it is set up and dispatched as the stack paragraph after the loop says. For every other sub-PR in the wave, the orchestrator creates the worktree first:

```
git worktree add .claude/worktrees/<slug>--<sub-name> -b <sub-type>/<slug>--<sub-name> <base-ref>
```

**The sub-branch name is derived, not invented.** It is `<sub-type>/<slug>--<sub-name>`: the parent's slug, so every branch of one feature sorts together in the remote's branch list, then `--<sub-name>` for this slice. `<sub-type>` is the commit type of *this* sub-PR's headline change, which is often not the parent's — a feature branch's slices routinely include a `refactor` that clears the way and a `test` that backfills coverage. The `<sub-name>` follows the same rule as the parent slug (`feature-dev-workflow:planning-a-feature` step 2): it names the slice's subject and effect in two to four words, never `part-2`, `phase-b`, or the plan's organizing label — those are positions in a plan the reader does not have, and inheriting them violates the naming firewall in `feature-dev-workflow:maintaining-architectural-coherence`.

Record the branch in the state file's PR-table row as you create it; the resumed-session contract in `feature-dev-workflow:resuming-a-feature` reads that row as ground truth.

When the state file's frontmatter has `sub_pr_target: main`, `<base-ref>` is `origin/main` for every sub-PR in every wave (fetch `origin/main` first so wave N+1 picks up the commits that wave N merged). When `sub_pr_target` is `feature-branch` (the default), `<base-ref>` is `<type>/<slug>` for default sub-PRs, `<type>/<slug>` after the stub PR merged (for `pre-merge stub PR` consumers), or the producer's branch (for `stub-on-producer-branch` consumers).

**A sub-PR based on another sub-PR's branch makes a stack, not a wave member, and so does the sub-PR it is based on.** That covers a `stub-on-producer-branch` consumer and, in either `sub_pr_target` model, any strictly sequential chain whose PRs open before their parents merge. Its branches, bases, propagation, and merges go through `gh stack`, which needs one dedicated worktree per stack (never the main checkout): follow `feature-dev-workflow:developing-a-feature` §Linear stacks of dependent PRs go through `gh stack`, including its availability check, before creating the consumer's branch. The orchestrator decides per layer whether it can be built in parallel, and runs every `gh stack` command. A sequential layer's subagent works in the stack's worktree, one layer at a time. A parallel layer's subagent works in a temporary worktree of its own, and the layer joins the stack after that worktree is removed (see that section's **Parallel layers**). Each isolation check names the worktree path and the layer's branch. A stack layer's subagent does not open its own PR (this replaces dispatch item 4 below); the orchestrator opens each stack PR as soon as that layer is ready, so a producer's PR is open before its stub consumer is dispatched, and links them as that section's **Opening the stack's PRs** says. Parallel sub-PRs based directly on `<base-ref>` stay in the worktree-per-sub-PR model below. A stack has at most one child per layer, so only one consumer can stack on a given producer. When two or more consumers are based on the same producer's branch, that is a fork, not a stack: one of them joins the producer's stack, and each other one waits until the producer has merged and then branches from `<base-ref>` like a `pre-merge stub PR` consumer.
...
4. **PR completion.** A stack layer's subagent skips this item and reports to the orchestrator, which opens the stack's PRs (see the stack paragraph above). A sequential layer's subagent reports its finished branch and head commit without pushing, because the orchestrator pushes the stack's worktree with `gh stack push`. A parallel layer's subagent pushes its branch from its temporary worktree first, then reports. Every other subagent, when the implementation is done and verified, invokes `feature-dev-workflow:opening-a-pull-request`. The base and body keyword depend on `sub_pr_target` in the state file: when `feature-branch`, use `--base <type>/<slug>` and `Towards #<sub-issue>` in the body (`Fixes`/`Closes` don't fire on non-default-branch merges; `Towards` keeps the issue open until the orchestrator closes it manually after the merge); when `main`, use `--base main` and the type-appropriate closing keyword in the body — `Fixes #<sub-issue>` for a bug sub-issue, `Closes #<sub-issue>` otherwise; either fires automatically on merge to the default branch (see `feature-dev-workflow:opening-a-pull-request` for the distinction). Epic closure in main mode is not handled by sub-PR keywords — the orchestrator closes the epic manually in Step 7. The subagent reports the PR URL back to the orchestrator.

Each dispatch prompt also carries this instruction for the subagent: **OPTIONAL SUB-SKILL:** `simple-english:simple-english` for the prose of your commit bodies, code and doc comments, and PR body. If it is available, load it before you draft and write the sentences under it. Where its formatting rules (headings, bold, lists, or its register for chat replies) differ from the host skill or its template, the host skill and its template win. It sets sentence style only: the commit subject keeps the project's commit convention, and the project's comment rules still decide whether a comment exists and what it may say. If it is not available, do not stop or wait for it: write short, plain, active sentences and say in your report that it was missing. The orchestrator, not the subagent, gives the user the install hint (`/plugin marketplace add AminBlg/SimpleEnglish`, then `/plugin install simple-english@simple-english`), at most once per session.

### 3. Update the state file as subagents start work

As each subagent surfaces its worktree path and branch, the orchestrator fills in the row in the state file's `## PRs / worktrees` table. When a subagent opens its draft PR, or the orchestrator opens a stack's PRs, the orchestrator fills in the PR column with the base ref (`#<num> → <type>/<slug>` or `#<num> → main`, per the `sub_pr_target` setting; a stack layer above the bottom shows its parent's branch) and flips status to `draft`.
...
- **Worktree topology.** Subagents live in their per-sub-PR worktrees; only the orchestrator's main feature worktree has `<type>/<slug>` checked out, so the merge naturally happens on the orchestrator's side.
...
3. **Approval gate, per the state file's `sub_pr_approval` mode.** Every gate covers the **bundle**: ready flip + merge + sub-issue close + state-file update. The close is bodyless (no `--comment` flag) — GitHub automatically cross-references the sub-issue from the merge commit via the sub-PR's body keyword, so no custom comment is needed and there's no "specific body about to land" for the close mutation.
   - **`autonomous`** (default) — proceed straight through the bundle in steps 4-6. The user opted into the mechanical bundle (review → ready flip → merge → bodyless close → state update) in `feature-dev-workflow:developing-a-feature` Step 2.
...
4. **Merge.** A draft cannot merge (`gh pr merge` and `gh stack merge` both refuse it), so first flip the PR ready through `feature-dev-workflow:opening-a-pull-request`'s steps for flipping a draft to ready; the approval gate above is the confirmation for that flip. Then run `gh pr merge <num> --merge` (or `--squash` / `--rebase` per project preference). A stack layer merges with `gh stack merge <num> --yes` and the same merge-method flag instead, followed by `gh stack sync` once the PR has actually merged (a merge queue can hold it first), and only when it is the lowest unmerged layer of its stack: `gh stack merge` also merges every unmerged layer below the one named, so a higher layer that passes its gates first waits until the layers below it have merged (see `feature-dev-workflow:developing-a-feature` §Linear stacks of dependent PRs go through `gh stack`). Before merging, push any local state-file commits to their remote — for `feature-branch` that is `origin/<type>/<slug>`; for `main` that is `origin/<type>/<slug>` as well (the orchestrator stays on the feature branch for state management). The merge itself lands on GitHub's remote, and the pull-back differs by target:
   - **`sub_pr_target: feature-branch`**: after the merge, `git -C <feature_worktree> fetch origin && git merge --ff-only origin/<type>/<slug>` to bring the merge commit back into the feature worktree. Keep local == origin at every merge boundary (unpushed local commits cause a "Not possible to fast-forward" failure; recover with `git rebase origin/<type>/<slug>`).
   - **`sub_pr_target: main`**: the merge lands on `main`. The orchestrator remains on `<type>/<slug>` for state file management; no pull-back into the feature worktree is needed. Before dispatching wave N+1, run `git fetch origin` so the next wave's worktrees branch from the freshly updated `origin/main`.
...
- **`sub_pr_target: feature-branch`**: the next step is the integration PR (`<type>/<slug>` → `main` with `Closes #<epic>`), which `feature-dev-workflow:developing-a-feature` Step 6 owns — opening and review-looping only; the merge to main itself is the user's (see that skill's merge guard).
